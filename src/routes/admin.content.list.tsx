...
function ContentListPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalBookmarks = Object.values(bookmarks).reduce((s, n) => s + n, 0);
    const reads = posts.map((p) => p.read_time_minutes ?? 0).filter((n) => n > 0);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {posts.length} post{posts.length === 1 ? "" : "s"} · {drafts} rascunho{drafts === 1 ? "" : "s"} · {stats.totalViews.toLocaleString("pt-BR")} visualizações
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filtered.map((p) => {
            ...
            return (
              <article key={p.id} className="group overflow-hidden rounded-2xl border border-border bg-card transition hover:shadow-md">
